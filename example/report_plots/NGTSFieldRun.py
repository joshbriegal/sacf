...
#    for obj in field:
        try:
            ...
            try:
                obj.clean_periods()
                fourier_ok = obj.check_fourier_ok()
                if fourier_ok:
                    is_moon = check_moon_detection(obj, field.new_moon_epoch)
                    if is_moon:
                        moon_ok = obj.remove_moon_and_check_ok()
                    else:
                        moon_ok = True
...
if __name__ == '__main__':
    ...
    ROOT_FILE = '/Users/joshbriegal/GitHub/GACF/example/report_plots'
...
#         logger.info("Splitting {} objects into {} fields of {} size (on {} cores)".format(num_objects, i+1, small_list_size, NUM_PROC))
    ...
    explicit_list = [(['report_plots/KIC_5110407_LC_quarter_5.dat',
                     'report_plots/KIC_5110407_LC_quarter_5_with_gaps.dat'], None, ROOT_FILE, FIELDNAME, CYCLE)]
    num_objects = np.sum([len(e[0]) for e in explicit_list])
