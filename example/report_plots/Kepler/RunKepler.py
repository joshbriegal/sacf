...
if __name__ == '__main__':
    ...
    plogger = logging.getLogger('Kepler Run')
    ...
    field = NF('Kepler', 'GACF', root_file='/Users/joshbriegal/GitHub/GACF/example/report_plots')
    field.objects = {
        0: NO(0, 'Kepler', 'GACF', field_filename=field.filename),
        1: NO(1, 'Kepler', 'GACF', field_filename=field.filename)
    }

    df_A = pd.read_csv('/Users/joshbriegal/GitHub/GACF/example/report_plots/KIC_5110407_LC_quarter_5.dat',
                       comment='#', sep='\s+',
                       names=['time', 'flux', 'flux_err'])  # , na_values='--').dropna().reset_index(drop=True)
    df_B = pd.read_csv('/Users/joshbriegal/GitHub/GACF/example/report_plots/KIC_5110407_LC_quarter_5_with_gaps.dat',
                       comment='#', sep='\s+',
                       names=['time', 'flux', 'flux_err'])  # , na_values='--').dropna().reset_index(drop=True)
    #
    # plogger.info(df_B.head(50))
    # exit()

    sort_idx = np.argsort(df_A['time'].tolist())
    field[0].timeseries_binned = list(np.array(map(float, df_A['time'].tolist()))[sort_idx] - df_A['time'][0])
    field[0].flux_binned = list(np.array(map(float, df_A['flux'].tolist()))[sort_idx])
    field[0].num_observations_binned = len(field[0].timeseries_binned)

    sort_idx = np.argsort(df_B['time'].tolist())
    field[1].timeseries_binned = list(np.array(map(float, df_B['time'].tolist()))[sort_idx] - df_B['time'][0])
    field[1].flux_binned = list(np.array(map(float, df_B['flux'].tolist()))[sort_idx])
    ...
    for obj in field:
        ...
        try:
            ...
            obj.lag_resolution = min(np.diff(obj.timeseries_binned))
            obj.min_lag = 0
            # obj.max_lag = 0
            ...
            try:
                fourier_ok = obj.check_fourier_ok(nlim=10)
                if fourier_ok:
                    obj.clean_periods(n=2)
    ...
    plt.savefig('Kepler_outputs.png')
    plt.show()
