...
freq_resolution = abs((1.0 / 20.001) - (1.0 / 20.000))
...
def find_unique_objects(fieldname_list=None):
    # find unique fields and tests, and take only fields with relevant TEST.
    fieldnames = {}
    if fieldname_list is None:
...
def load_most_likely_periods(fieldname_list=None, clean_periods=True):
    filename_pattern = re.compile(r'^NGTS_Field_NG\d+?[+|-]\d+?_pdic.json$')
    fieldnames = {}
    for f in os.listdir(ROOT_DIR):
        match = re.match(field_pattern, f)
        if match:
            if fieldname_list is not None:
                if match.string not in fieldname_list:
                    continue
            fieldnames[match.string] = []
            
    for fieldname in fieldnames:
        print 'Finding data for field {}'.format(fieldname)
        for f in os.listdir(os.path.join(ROOT_DIR, fieldname)):
            match = None
            match = re.match(filename_pattern, f)
            if match:
                print 'Loading from {}'.format(match.string)
                filename = os.path.join(*[ROOT_DIR, fieldname, match.string])
                with open(filename, 'r') as of:
                    fieldnames[fieldname] = json.load(of, object_hook=jsonKeys2int)
                    print 'Loaded {} objects for field {}'.format(len(fieldnames[fieldname]), fieldname)
                break
    
    unique_objs = {}
    for fieldname, obj_data in fieldnames.iteritems():
        print 'Processing objects from field {}'.format(fieldname)
        field_data = []
        for obj, periods in obj_data.iteritems(): 
            if periods is not None:
                for period in periods:
                    field_data.append((int(obj), 1.0 / float(period), None))
        bin_medians, binned_data = create_field_histogram(field_data, freq_resolution)
        
        num_periods = len(field_data)

        timeseries_length = 250.0
        ...
        if clean_periods:
            ...
            final_bins = field_data
            
        num_periods_out = len(final_bins)

        unique_objs[fieldname] = sorted(final_bins, key=lambda d: d[0])
        
        print 'Removed {} of {} periods'.format(num_periods-num_periods_out, num_periods)
                
    return unique_objs, fieldnames
...
if __name__ == '__main__':
    ...
    unique_objects, data = load_most_likely_periods(clean_periods=True)#'NG2346-3633')
#    unique_objects_clean, data_clean = load_most_likely_periods(clean_periods=True)
    
    fields = []
    for fieldname, obj_data in unique_objects.iteritems():
        field = NGTSField(fieldname, 'CYCLE1807', root_file=ROOT_DIR, object_list = [int(d[0]) for d in obj_data])
        for obj in field:
            obj.periods_ok = data[fieldname][int(obj.obj)]
        fields.append(field)
    for f in fields:
        print "{}, num objects {}".format(f, f.num_objects)
...
#
    mamajek_comparison(fields)
    
    with open('unique_objects.json', 'w') as f:
